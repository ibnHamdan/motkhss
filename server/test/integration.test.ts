...
describe('integration test', () => {
  ...
  it('creates a opportunity and returns it in list and get', async () => {
    const postTitle = 'first opportunity';
    ...
    let postId: string;
    {
      const { method, url } = ENDPOINT_CONFIGS.listOpportunities;
    ...
    {
      const { method, url } = ENDPOINT_CONFIGS.getOpportunity;
      const opportunity = await client[method](url.replace(':id', postId)).expect(200);
      expect(opportunity.body.opportunity.id).toBe(postId);
      expect(opportunity.body.opportunity.title).toBe(postTitle);
  ...
  it('comments on opportunity, multiple times, returns list of comments, deletes 1st comment', async () => {
  ...
  const get1stOopportunity = async () => {
    const { method, url } = ENDPOINT_CONFIGS.listOpportunities;
    const postList = await client[method](url).expect(200);
